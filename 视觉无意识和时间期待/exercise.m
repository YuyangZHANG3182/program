...
paramatrix0=repmat(paramatrix0,3,1);%复制3次
...
paramatrix0(:,3)=710;
paramatrix0(:,4)=35;
paramatrix0(:,5)=repmat([71*5 71*10 71]',4,1);
elseif cond==3
paramatrix0(:,2)=3;
paramatrix0(:,3)=repmat([71*6 71*10 71]',4,1);    
paramatrix0(:,4)=35;
...
while GetSecs() - onset < maxResponseTime
 ...
 % 判断正确性 (目标方向: 45对应右箭头(1), -45对应Z键(0))
    ...
    if paramatrix(i,10) == 0
    % 生成1000Hz蜂鸣声
    fs = 44100;                 % 采样率
    duration = 0.3;             % 持续时间
    t = 0:1/fs:duration;        % 时间向量
    yy = sin(2*pi*1000*t);       % 生成正弦波
    sound(yy, fs);               % 播放声音
    end
...
dataTable.Subject = repmat({subname}, 12, 1);
filename = sprintf('%s_text_cond%d.csv', subname, cond);
